...
namespace truYumApi
{
    ...
    {
        public static string sqlDataSource = "Server=.;Database=MenuItems;Trusted_Connection=True;MultipleActiveResultSets=true;";
        ...
        public static void Update(int id, MenuItem menuitem)
        {
            ...
            using (SqlConnection con = new SqlConnection(sqlDataSource))
            {
                SqlCommand cmd = new SqlCommand("Update MenuItem set Id=@Id,Name=@Name,Price=@Price,Active=@Active,DateOfLaunch=@DateOfLaunch,CategoryId=@CategoryId,CategoryName=@CategoryName,Freedelivery=@FreeDelivery where Id=id", con);
                ...
                cmd.Parameters.AddWithValue("@CategoryName", menuitem.CategoryName);
